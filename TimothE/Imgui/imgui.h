...
namespace ImGui
{
    ...
    IMGUI_API void          SetWindowFontScale(float _tileScale);                                            // [OBSOLETE] set font scale. Adjust IO.FontGlobalScale if you want to scale all windows. This is an old API! For correct scaling, prefer to reload font + rebuild ImFontAtlas + call style.ScaleAllSizes().
...
struct ImGuiPayload
{
    ...
    ImGuiPayload()  { ClearLayer(); }
    void ClearLayer()    { SourceId = SourceParentId = 0; Data = NULL; DataSize = 0; memset(DataType, 0, sizeof(DataType)); DataFrameCount = -1; Preview = Delivery = false; }
...
struct ImGuiTextFilter
{
    ...
    void                ClearLayer()          { InputBuf[0] = 0; Build(); }
...
struct ImDrawListSplitter
{
    ...
    inline void                 ClearLayer() { _Current = 0; _Count = 1; } // Do not clear Channels[] so our allocations are reused next frame
...
struct ImDrawData
{
    ...
    // Functions
    ImDrawData()    { ClearLayer(); }
    void ClearLayer()    { memset(this, 0, sizeof(*this)); }     // The ImDrawList are owned by ImGuiContext!
...
struct ImFontGlyphRangesBuilder
{
    ...
    ImFontGlyphRangesBuilder()              { ClearLayer(); }
    inline void     ClearLayer()                 { int size_in_bytes = (IM_UNICODE_CODEPOINT_MAX + 1) / 8; UsedChars.resize(size_in_bytes / (int)sizeof(ImU32)); memset(UsedChars.Data, 0, (size_t)size_in_bytes); }
...
struct ImFontAtlas
{
    ...
    IMGUI_API void              ClearLayer();                    // Clear all input and output.
...
struct ImFont
{
    ...
    IMGUI_API const char*       CalcWordWrapPositionA(float _tileScale, const char* text, const char* text_end, float wrap_width) const;
