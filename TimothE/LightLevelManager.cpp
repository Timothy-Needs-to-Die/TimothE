	...
	if (_lightSources.size() > 0)
	{
		...
		for (auto const& source : _lightSources)
		{
			for (int i = 0; i < 6; i++)
			{
				...
				xMin = tileMap->GetTileAtWorldPos(i, glm::vec2(source.worldPos.x - (source.range * tileSize), source.worldPos.y))->pos.x;
				xMax = tileMap->GetTileAtWorldPos(i, glm::vec2(source.worldPos.x + (source.range * tileSize), source.worldPos.y))->pos.x;
				yMin = tileMap->GetTileAtWorldPos(i, glm::vec2(source.worldPos.x, source.worldPos.y - (source.range * tileSize)))->pos.y;
				yMax = tileMap->GetTileAtWorldPos(i, glm::vec2(source.worldPos.x, source.worldPos.y + (source.range * tileSize)))->pos.y;

				//xMin = tileMap->GetTileAtWorldPos(i, glm::vec2(2.0f, 2.0f))->pos.x - 30.0f;
				//xMax = tileMap->GetTileAtWorldPos(i, glm::vec2(2.0f, 2.0f))->pos.x + 30.0f;
				//yMin = tileMap->GetTileAtWorldPos(i, glm::vec2(2.0f, 2.0f))->pos.y - 30.0f;
				//yMax = tileMap->GetTileAtWorldPos(i, glm::vec2(2.0f, 2.0f))->pos.y + 30.0f;

				for (float y = yMin; y <= yMax; y += tileSize)
				{
					for (float x = xMin; x <= xMax; x += tileSize)
					{
						TileData* tile = tileMap->GetTileAtWorldPos(i, glm::vec2(x, y));
						...
						if (tile->lightLevel + affectiveValue < 2)
						{
							tile->lightLevel = 2;
							continue;
						}
